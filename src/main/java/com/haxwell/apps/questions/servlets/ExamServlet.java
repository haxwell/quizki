...
public class ExamServlet extends AbstractHttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// in case they press a button to delete a question.. this will be
		// null..
		if (button == null)
			button = "";
		...
		if (button.equals("Add Exam") || button.equals("Update Exam")) {
			...
			int quantity = Integer.parseInt(getIdAppendedToRequestParameter(
					request, "quantity"));
	...
	private PaginationData getQuestionPaginationData(HttpServletRequest request) {
		return (PaginationData) request.getSession().getAttribute(
				Constants.QUESTION_PAGINATION_DATA);
	}

	private void setQuestionPaginationData(HttpServletRequest request,
			PaginationData pd) {
		request.getSession().setAttribute(Constants.QUESTION_PAGINATION_DATA,
				pd);
	}

	private void clearMRUFilterSettings(HttpServletRequest request) {
		request.getSession()
				.setAttribute(Constants.MRU_FILTER_DIFFICULTY, null);
		request.getSession().setAttribute(Constants.MRU_FILTER_TEXT, null);
		request.getSession()
				.setAttribute(Constants.MRU_FILTER_TOPIC_TEXT, null);
	}
	...
	private boolean handleAddExamButtonPress(HttpServletRequest request,
		...
		if (validation(request, errors)) {
			...
			successes
					.add("Exam '"
							+ examObj.getTitle()
							+ "' has been saved successfully. <a href=\"/secured/exam.jsp?examId="
							+ id
							+ "\">(edit it)</a>, <a href=\"/beginExam.jsp?examId="
							+ id + "\">(take it)</a>");
			...
			request.getSession().setAttribute(Constants.MRU_FILTER_DIFFICULTY,
					DifficultyConstants.GURU);

			EventDispatcher.getInstance().fireEvent(request,
					EventConstants.EXAM_WAS_PERSISTED);
	...
	private void saveExamSelectedQuestionIdsInSession(HttpServletRequest request) {
		Collection<Long> selectedQuestionIds = CollectionUtil
				.getListOfIds(getExamBean(request).getQuestions());
		request.getSession().setAttribute(
				Constants.CURRENT_EXAM_SELECTED_QUESTION_IDS,
				selectedQuestionIds);
	}
	...
	private Exam getExamBean(HttpServletRequest request) {
		Exam exam = (Exam) request.getSession().getAttribute(
				Constants.CURRENT_EXAM);

		if (exam == null) {
			new InitializeNewExamInSessionAction().doAction(request, null);
			exam = (Exam) request.getSession().getAttribute(
					Constants.CURRENT_EXAM);
		}
