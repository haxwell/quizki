...
function completely(container, config) {
    ...
    config.showDropDown = 					config.showDropDown === undefined ? true : config.showDropDown;
    config.onKeyPress = 					config.onKeyPress; // no default value, it is what it is
    config.onDeleteKeyPress = 				config.onDeleteKeyPress; 
    ...
    **/
    ...
    var keyDownHandler = function(e) {
        ...
        if (config.onKeyPress !== undefined) 
        	config.onKeyPress(e);
        ...
        if (keyCode == 46) { // delete
        	var index = dropDownController.position();
        	var item = rs.options[index];
        	
        	rs.options.splice(index, 1);
        	
        	config.onDeleteKeyPress(item, index);
