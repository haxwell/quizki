<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
	...
	<form id="adminProfileExamForm" action="/secured/AdminProfileExamsServlet">
		...
		<c:forEach var="exam" items="${fa_listofexamstobedisplayed}">
			...
			<jsp:text><![CDATA[<tr class="rowHighlight">]]></jsp:text>
				...
				<div class="examButtonDiv">
					<input type="submit" value="Edit" id="exam_edit_button_${counter}" name="examButton_${exam.id}"/>
					<input type="submit" value="Delete" id="exam_delete_button_${counter}" name="examButton_${exam.id}"/>
					<input type="submit" value="Detail Exam" id="exam_detail_button_${counter}" name="examButton_${exam.id}"/>							
				</div>
				</td>
