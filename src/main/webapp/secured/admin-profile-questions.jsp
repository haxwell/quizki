...
<%@ taglib uri="http://quizki.com/tld/qfn" prefix="qfn" %>
	...
	<form id="adminProfileQuestionForm" action="/secured/AdminProfileQuestionsServlet">
				...
				<tr>
					...
					<td >
						<select name="difficultyFilter" title="Do not include any questions more difficult than.." style="width:100%;">
							...
							<c:choose><c:when test="${mruFilterDifficulty == 3}"><option value="senior" selected="selected">Senior</option></c:when><c:otherwise><option value="senior" >Senior</option></c:otherwise></c:choose>
			...
			<jsp:text><![CDATA[<tr class="rowHighlight">]]></jsp:text>
				...
				<td>
					...
					<div class="questionButtonDiv">
						<input type="submit" value="Edit" id="edit_button_${counter}" name="questionButton_${question.id}"/>
						<input type="submit" value="Delete" id="delete_button_${counter}" name="questionButton_${question.id}"/>
					</div>
				</td>
